...
void Lamphsv::add_hue_angle(float angle) 
{
  m_color_hsv.h+=angle; 
...
void Lamphsv::add_saturation(float increment) 
{
...
void Lamphsv::multiply_value(float fact) 
{
  m_color_hsv.v*=fact; 
...
void Lamphsv::set_hue_angle(float angle) 
{
  m_color_hsv.h=angle; 
  while(m_color_hsv.h>=360.) {m_color_hsv.h-=360.0;}; 
  while(m_color_hsv.h<0.0) {m_color_hsv.h+=360.0;};
...
void Lamphsv::set_saturation(float saturation) 
{
...
void Lamphsv::set_value(float value) 
{
  m_color_hsv.v=value; 
  if(m_color_hsv.v>=1.0) m_color_hsv.v=1.0; 
  if(m_color_hsv.v<0.0) m_color_hsv.v=0.0;
...
void Lamphsv::set_hsv(float h,float s,float v)
{
  m_color_hsv.h=h; 
...
void Lamphsv::trace_rgb(t_color_rgb_int color_rgb )
{
...
t_color_rgb_int Lamphsv::get_color_rgb(float maximum_value)
{
